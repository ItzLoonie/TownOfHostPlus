    ...
    class ControllerManagerUpdatePatch
    {
        ...
        public static void Postfix(ControllerManager __instance)
        {
            ...
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                if (Input.GetKeyDown(KeyCode.F6) && GameStates.IsMeeting && GameStates.IsOnlineGame)
                {
                    MeetingHud.Instance.RpcClearVote(AmongUsClient.Instance.ClientId);
                }
                if (Input.GetKeyDown(KeyCode.Return) && Input.GetKey(KeyCode.P) && Input.GetKey(KeyCode.LeftShift) && GameStates.IsInGame)
                {
                    if (ShipStatus.Instance != null)
                        foreach (var pc in PlayerControl.AllPlayerControls)
                        {
                    ...
                    {
                        MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.EndGame, Hazel.SendOption.Reliable, -1);
                        writer.Write((int)CustomWinner.TooManyPlayersLeft);
                        AmongUsClient.Instance.FinishRpcImmediately(writer);
                        RPC.TooManyPlayersLeft();
                    }, 0.5f, "Host Force End Game");
                }
                //ミーティングを強制終了
            ...
            if (Main.CachedDevMode)
            {
                ...
                if (Input.GetKey(KeyCode.RightShift) && Input.GetKeyDown(KeyCode.N) && Input.GetKeyDown(KeyCode.Return))
                {
                    ...
                    bot.RpcSetVisor(PlayerControl.LocalPlayer.CurrentOutfit.VisorId);
                    bot.RpcSetLevel(419);
