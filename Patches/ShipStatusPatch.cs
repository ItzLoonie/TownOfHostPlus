...
namespace TownOfHost
{
    ...
    class RepairSystemPatch
    {
            ...
            [HarmonyArgument(2)] byte amount)
        {
            ...
            if (player.Is(CustomRoles.QuickFix))
                QuickFix.RepairSystem(__instance, systemType, amount);
            ...
            if (player.Is(CustomRoles.Sheriff) || player.Is(CustomRoles.Deputy) || player.Is(CustomRoles.NeutWitch) || player.Is(CustomRoles.Consigliere) || player.Is(CustomRoles.Vindicator) || player.Is(CustomRoles.PoisonMaster) || player.Is(CustomRoles.Investigator) || player.Is(CustomRoles.Escort) || player.Is(CustomRoles.Crusader) || player.Is(CustomRoles.Parasite) || player.Is(CustomRoles.Marksman) || player.Is(CustomRoles.BloodKnight) || player.Is(CustomRoles.Arsonist) || player.Is(CustomRoles.Werewolf) || player.Is(CustomRoles.AgiTater) || player.Is(CustomRoles.TheGlitch) || player.GetRoleType() == RoleType.Coven || player.Is(CustomRoles.PlagueBearer) || player.Is(CustomRoles.Pestilence) || player.Is(CustomRoles.Juggernaut) || player.Is(CustomRoles.Predator) || player.Is(CustomRoles.Wraith) || ((player.Is(CustomRoles.Jackal) || player.Is(CustomRoles.Sidekick)) && !Options.JackalCanUseSabotage.GetBool()) || Main.Grenaiding)
            {
    ...
    class SwitchSystemRepairPatch
    {
        public static void Postfix(SwitchSystem __instance, [HarmonyArgument(0)] PlayerControl player, [HarmonyArgument(1)] byte amount)
        {
            ...
            if (player.Is(CustomRoles.QuickFix))
                QuickFix.SwitchSystemRepair(__instance, amount);
