...
namespace TownOfHost
{
    ...
    class PingTrackerPatch
    {
        ...
        static void Postfix(PingTracker __instance)
        {
            ...
            if (Main.CachedDevMode) __instance.text.text += "\r\n" + Helpers.ColorString(Color.green, "Debug Mode");
            ...
            else
            {
                ...
                if (Options.IsStandardHAS && !CustomRoles.Sheriff.IsEnable() && !CustomRoles.Mercenary.IsEnable() && CustomRoles.Egoist.IsEnable()) __instance.text.text += $"\r\n" + Helpers.ColorString(Color.red, GetString("Warning.EgoistCannotWin"));
            }
        }
        ...
        class VersionShowerPatch
        {
            ...
            {
                Main.credentialsText = $"\r\n<color={Main.modColor}>Town Of Host Plus {Main.ModVersion}</color>";
                Main.versionText = $"\r\nTown Of Host Plus {Main.ModVersion}";
                if (Main.NewYears)
                {
                    Main.credentialsText += "\r\nHAPPY NEW YEAR!";
                    Main.versionText += "\r\nHAPPY NEW YEAR!";
                }
                if (Main.IsSpookyMonth)
                {
                    Main.credentialsText += "\r\nIt's a Spooky Month!";
                    Main.versionText += "\r\nIt's a Spooky Month!";
                }
