    ...
    class AddTasksFromListPatch
    {
            ...
            [HarmonyArgument(4)] Il2CppSystem.Collections.Generic.List<NormalPlayerTask> unusedTasks)
        {
            ...
            for (var i = 0; i < unusedTasks.Count; i++)
            {
                ...
                if (task.TaskType == TaskTypes.OpenWaterways && Options.DisableWaterwheels.GetBool()) disabledTasks.Add(task);
                if (task.TaskType == TaskTypes.RebootWifi && Options.DisableWaitingTasks.GetBool()) disabledTasks.Add(task);
                if (task.TaskType == TaskTypes.InspectSample && Options.DisableWaitingTasks.GetBool()) disabledTasks.Add(task);
                if (task.TaskType == TaskTypes.RunDiagnostics && Options.DisableWaitingTasks.GetBool()) disabledTasks.Add(task);
                if (task.TaskType == TaskTypes.VentCleaning && Options.DisableCleanVent.GetBool()) disabledTasks.Add(task);
            }
