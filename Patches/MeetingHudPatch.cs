...
namespace TownOfHost
{
    ...
    class CheckForEndVotingPatch
    {
        public static bool Prefix(MeetingHud __instance)
        {
            ...
            try
            {
                foreach (var pva in __instance.playerStates)
                {
                    ...
                    if (pc == null) continue;
                    //死んでいないディクテーターが投票済み
                    if (pc.Is(CustomRoles.Dictator) && pva.DidVote && pc.PlayerId != pva.VotedFor && pva.VotedFor < 253 && !pc.Data.IsDead && !IsPhantom(pva.VotedFor) && !IsWraith(pva.VotedFor))
                    {
                        ...
                        if (voteTarget.GetCustomRole() is CustomRoles.Chainlink)
                        {
                            PlayerControl chained = Main.ChainedPlayers.ToArray().Where(pc => pc.PlayerId == voteTarget.PlayerId).FirstOrDefault();
                            ...
                            //   if (Options.LoversDieTogether.GetBool())
                            {
                                foreach (var lp in Main.ChainedPlayers)
                                {
                                    ...
                                    {
                                        if (!Main.AfterMeetingDeathPlayers.ContainsKey(lp.PlayerId))
                                            Main.AfterMeetingDeathPlayers.Add(lp.PlayerId, PlayerState.DeathReason.Choked);
                                    }
                ...
                for (var i = 0; i < __instance.playerStates.Length; i++)
                {
                    ...
                    if (voter.GetCustomRole() is CustomRoles.Oracle or CustomRoles.Bodyguard or CustomRoles.Medic)
                    {
                        if (Main.CurrentTarget.ContainsKey(ps.TargetPlayerId))
                            if (Main.CurrentTarget[ps.TargetPlayerId] == 255 && ps.VotedFor != ps.TargetPlayerId && ps.VotedFor != 253 && ps.VotedFor != 254 && ps.VotedFor != 255 && !Main.HasTarget[ps.TargetPlayerId])
                            {
                                ...
                                Utils.SendMessage($"You have locked in your target.\nYour target is: {Utils.GetPlayerById(Main.CurrentTarget[ps.TargetPlayerId]).GetRealName(true)}", ps.TargetPlayerId);
                            }
                    }
                    ...
                    if (IsEvilMayor(ps.TargetPlayerId) && !Options.MayorVotesAppearBlack.GetBool())
                    {
                    ...
                    if (IsTrickster(ps.TargetPlayerId) && !Options.MayorVotesAppearBlack.GetBool())
                    {
                        for (var i2 = 0; i2 < Options.TricksterVotes.GetFloat(); i2++)
                        {
                            statesList.Add(new MeetingHud.VoterState()
                            {
                                VoterId = ps.TargetPlayerId,
                    ...
                    if (IsVindicated(ps.TargetPlayerId) && !Options.MayorVotesAppearBlack.GetBool())
                    {
                        for (var i2 = 0; i2 < Options.VindicatorVotes.GetFloat(); i2++)
                        {
                            statesList.Add(new MeetingHud.VoterState()
                            {
                                VoterId = ps.TargetPlayerId,
                    ...
                    if (IsGlitch(ps.TargetPlayerId) && !Options.MayorVotesAppearBlack.GetBool())
                    {
                        for (var i2 = 0; i2 < Options.VindicatorVotes.GetFloat(); i2++)
                        {
                            statesList.Add(new MeetingHud.VoterState()
                    ...
                    if (IsParasite(ps.TargetPlayerId) && !Options.MayorVotesAppearBlack.GetBool())
                    {
                        for (var i2 = 0; i2 < Options.ParasiteVotes.GetFloat(); i2++)
                        {
                            statesList.Add(new MeetingHud.VoterState()
                            {
                                VoterId = ps.TargetPlayerId,
                    ...
                    if (IsAmplified(ps.TargetPlayerId) && !Options.MayorVotesAppearBlack.GetBool())
                    {
                        //  for (var i2 = 0; i2 < Options.ParasiteVotes.GetFloat(); i2++)
                        {
                            statesList.Add(new MeetingHud.VoterState()
                ...
                if (!Utils.GetPlayerById(exileId).Is(CustomRoles.Spellcaster))
                {
                    foreach (var p in Main.SpelledPlayer)
                    {
                        ...
                        if (CustomRoles.Chainlink.IsEnable() && Main.isChainedDead == false && Main.ChainedPlayers.Find(lp => lp.PlayerId == p.PlayerId) != null)
                        {
                            PlayerControl chained = Main.ChainedPlayers.ToArray().Where(pc => pc.PlayerId == p.PlayerId).FirstOrDefault();
                            Main.ChainedPlayers.Remove(chained);
                            Main.isChainedDead = true;
                            //       if (Options.LoversDieTogether.GetBool())
                            {
                ...
                if (CustomRoles.Chainlink.IsEnable() && Main.isChainedDead == false && Main.ChainedPlayers.Find(lp => lp.PlayerId == exileId) != null)
                {
                    PlayerControl chained = Main.ChainedPlayers.ToArray().Where(pc => pc.PlayerId == exileId).FirstOrDefault();
                    Main.ChainedPlayers.Remove(chained);
                    Main.isChainedDead = true;
                    //       if (Options.LoversDieTogether.GetBool())
                    {
        ...
        public static bool IsTrickster(byte id)
        {
            var player = PlayerControl.AllPlayerControls.ToArray().Where(pc => pc.PlayerId == id).FirstOrDefault();
            return player != null && player.Is(CustomRoles.Trickster);
        }
        ...
        public static bool IsVindicated(byte id)
        {
            var player = PlayerControl.AllPlayerControls.ToArray().Where(pc => pc.PlayerId == id).FirstOrDefault();
            return player != null && player.Is(CustomRoles.Vindicator);
        }
        public static bool IsGlitch(byte id)
        {
            var player = PlayerControl.AllPlayerControls.ToArray().Where(pc => pc.PlayerId == id).FirstOrDefault();
            return player != null && player.Is(CustomRoles.Glitch);
        }
        public static bool IsParasite(byte id)
        {
            var player = PlayerControl.AllPlayerControls.ToArray().Where(pc => pc.PlayerId == id).FirstOrDefault();
            return player != null && player.Is(CustomRoles.Parasite);
        }
        public static bool IsEvilMayor(byte id)
        {
            var player = PlayerControl.AllPlayerControls.ToArray().Where(pc => pc.PlayerId == id).FirstOrDefault();
            return player != null && player.Is(CustomRoles.Pickpocket);
        }
        ...
        public static bool IsAmplified(byte id)
        {
            var player = PlayerControl.AllPlayerControls.ToArray().Where(pc => pc.PlayerId == id).FirstOrDefault();
            return player != null && player.Is(CustomRoles.Amplify);
        }
    }
        ...
        public static Dictionary<byte, int> CustomCalculateVotes(this MeetingHud __instance)
        {
            ...
            for (int i = 0; i < __instance.playerStates.Length; i++)
            {
                ...
                if (ps.VotedFor is not ((byte)252) and not byte.MaxValue and not ((byte)254))
                {
                    ...
                    if (CheckForEndVotingPatch.IsTrickster(ps.TargetPlayerId)) VoteNum += Options.TricksterVotes.GetInt();
                    if (CheckForEndVotingPatch.IsWraith(ps.TargetPlayerId)) VoteNum += -1;
                    if (CheckForEndVotingPatch.IsVindicated(ps.TargetPlayerId)) VoteNum += Options.VindicatorVotes.GetInt();
                    //        if (CheckForEndVotingPatch.IsGlitch(ps.TargetPlayerId)) VoteNum += Options.GlitchVotes.GetInt();
                    if (CheckForEndVotingPatch.IsAmplified(ps.TargetPlayerId)) VoteNum += 1;
                    if (CheckForEndVotingPatch.IsParasite(ps.TargetPlayerId)) VoteNum += Options.ParasiteVotes.GetInt();
    ...
    class MeetingHudStartPatch
    {
        ...
        public static void Postfix(MeetingHud __instance)
        {
                ...
                try
                {
                    if (!Options.RolesLikeToU.GetBool())
                    {
                        ...
                        roleTextMeeting.enabled =
                            pva.TargetPlayerId == PlayerControl.LocalPlayer.PlayerId || Main.rolesRevealedNextMeeting.Contains(pva.TargetPlayerId) || (PlayerControl.LocalPlayer.GetCustomRole().IsImpostor() && Options.ImpostorKnowsRolesOfTeam.GetBool() && pc.GetCustomRole().IsImpostor()) || (PlayerControl.LocalPlayer.GetCustomSubRole().IsLover() && Options.LoversKnowRoleOfOtherLover.GetBool() && pc.GetCustomSubRole().IsLover()) ||
                    ...
                    else
                    {
                        ...
                        bool continues = pva.TargetPlayerId == PlayerControl.LocalPlayer.PlayerId || Main.rolesRevealedNextMeeting.Contains(pva.TargetPlayerId) || (PlayerControl.LocalPlayer.GetCustomRole().IsImpostor() && Options.ImpostorKnowsRolesOfTeam.GetBool() && pc.GetCustomRole().IsImpostor()) || (PlayerControl.LocalPlayer.GetCustomSubRole().IsLover() && Options.LoversKnowRoleOfOtherLover.GetBool() && pc.GetCustomSubRole().IsLover()) ||
                            (Main.VisibleTasksCount && PlayerControl.LocalPlayer.Data.IsDead && Options.GhostCanSeeOtherRoles.GetBool()) || (PlayerControl.LocalPlayer.GetCustomRole().IsCoven() && Options.CovenKnowsRolesOfTeam.GetBool() && pc.GetCustomRole().IsCoven()) || (AmongUsClient.Instance.AmHost && PlayerControl.LocalPlayer.Is(CustomRoles.GM));
                    ...
                    foreach (var pc in PlayerControl.AllPlayerControls)
                    {
                        ...
                        switch (role.GetRoleType())
                        {
                            case RoleType.Crewmate:
                                ...
                                if (role is CustomRoles.Sheriff or CustomRoles.Deputy or CustomRoles.Veteran or CustomRoles.Bodyguard or CustomRoles.Crusader or CustomRoles.Child or CustomRoles.Bastion or CustomRoles.Demolitionist or CustomRoles.Vigilante) badPlayers.Add(pc);
                                break;
                            case RoleType.Impostor:
                                if (role is CustomRoles.Trickster) break;
                                badPlayers.Add(pc);
                                isGood[pc.PlayerId] = false;
                                break;
                    ...
                    _ = new LateTask(() =>
                    {
                        ...
                        foreach (var pc in PlayerControl.AllPlayerControls)
                        {
                            ...
                            if (Main.rolesRevealedNextMeeting.Contains(pc.PlayerId))
                                Utils.SendMessage("Through the powers of an Oracle, your role has been revealed to everyone!", pc.PlayerId);
                        }
            ...
            if (AmongUsClient.Instance.AmHost)
                ...
                foreach (var pb in PlayerControl.AllPlayerControls)
            {
                if (pb == null || pb.Data.IsDead || pb.Data.Disconnected) continue;
                if (pb.GetCustomRole() != CustomRoles.PlagueBearer) continue;
                if (pb.GetCustomRole() != CustomRoles.Pestilence) continue;
                if (pb.IsInfectDone())
                    Utils.SendMessage("A plague has consumed the crew summoning Pestilence, Horseman of the Apocalypse!");
            }
                ...
                try
                {
                    ...
                    switch (seer.GetCustomRole().GetRoleType())
                    {
                        case RoleType.Impostor:
                            ...
                            if (target.Is(CustomRoles.Undercover) && Options.ImpostorsKnowUndercover.GetBool())
                                pva.NameText.text = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Impostor), pva.NameText.text);
                            break;
                        case RoleType.Coven:
                            if (target.Is(CustomRoles.Snitch) && //変更対象がSnitch
                    ...
                    switch (seer.GetCustomSubRole())
                    {
                        case CustomRoles.Forensic:
                            if (target.Data.IsDead) //変更対象が死人
                                pva.NameText.text += $"({Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Forensic), target.GetDeathReason())})";
                            break;
                        case CustomRoles.Undercover:
                            if (target.Is(RoleType.Impostor) && Options.UndercoverKnowsImpostors.GetBool())
                                pva.NameText.text = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Impostor), pva.NameText.text);
                            break;
                        case CustomRoles.Necroview:
                            if (target.Is(RoleType.Crewmate) && target.Data.IsDead)
                                pva.NameText.text = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.SpeedBooster), pva.NameText.text);
                            if (target.Is(RoleType.Impostor) && target.Data.IsDead)
                                pva.NameText.text = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Impostor), pva.NameText.text);
                            if (target.Is(RoleType.Madmate) && target.Data.IsDead)
                                pva.NameText.text = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Madmate), pva.NameText.text);
                            if (target.Is(RoleType.Neutral) && target.Data.IsDead)
                                pva.NameText.text = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.SchrodingerCat), pva.NameText.text);
                            if (target.Is(RoleType.Coven) && target.Data.IsDead)
                                pva.NameText.text = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.NeutWitch), pva.NameText.text);
                            break;
                    ...
                    switch (seer.GetCustomRole())
                    {
                        ...
                        case CustomRoles.Trickster:
                            if (target.Data.IsDead) //変更対象が死人
                                pva.NameText.text += $"({Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Trickster), target.GetDeathReason())})";
                            break;
                        ...
                        case CustomRoles.Lawyer:
                            if (Main.LawyerTarget.TryGetValue(seer.PlayerId, out var clientId) && target.PlayerId == clientId) //targetがValue
                            ...
                            if (Consigliere.hasSeered[target.PlayerId] == true)
                            {
...

                                {
                                    if (Consigliere.IsImp(target))
                                    {
                                        ...
                                        {
                                            pva.NameText.color = Utils.GetRoleColor(CustomRoles.Consigliere);
                                        }
                                    ...
                                    if (Consigliere.IsCoven(target))
                                    {
                                        ...
                                        {
                                            pva.NameText.color = Utils.GetRoleColor(CustomRoles.NeutWitch);
                                        }
                    ...
                    switch (target.GetCustomSubRole())
                    {
                        case CustomRoles.LoversRecode:
                            ...
                            break;
                        case CustomRoles.Chainlink:
                            if (seer.Is(CustomRoles.Chainlink) || seer.Data.IsDead)
                                pva.NameText.text += Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Chainlink), "●");
                            break;
                    }
                    ...
                    switch (target.GetCustomRole())
                    {
                        ...
                        case CustomRoles.Mimicker:
                            if (seer.Is(CustomRoles.Mimicker))
                                pva.NameText.color = Utils.GetRoleColor(CustomRoles.Mimicker);//変更対象の名前をジャッカル色にする
                            break;
                    ...
                    {
                        if (target != null && target.GetCustomRole().CanBeFoundBySnitch()) //変更先がインポスター
                                                                                           //   if (target.Is(CustomRoles.Trickster)) break;


                            pva.NameText.color = Utils.GetRoleColor(target.GetCustomRole()); //変更対象の名前を赤くする
                    }
