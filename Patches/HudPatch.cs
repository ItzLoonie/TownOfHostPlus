...
namespace TownOfHost
{
    ...
    class HudManagerPatch
    {
        ...
        public static void Postfix(HudManager __instance)
        {
            ...
            if (SetHudActivePatch.IsActive && player.IsAlive())
            {
                switch (player.GetCustomRole())
                {
                    ...
                    case CustomRoles.Cultist:
                        __instance.KillButton.OverrideText("CONVERT");
                        break;
                    ...
                    case CustomRoles.Glitch:
                        __instance.SabotageButton.OverrideText("CORRUPT");
                        break;
                    case CustomRoles.Wisp:
                        __instance.AbilityButton.OverrideText("WISP");
                        break;
                    ...
                    case CustomRoles.Eradicator:
                        __instance.KillButton.OverrideText("INJECT");
                        break;
                    ...
                    case CustomRoles.Arsonist:
                        __instance.ImpostorVentButton.OverrideText("IGNITE");
                ...
                else if (player.Is(CustomRoles.Spellcaster))
                {
                    //魔女用処理
                ...
                else if (player.Is(CustomRoles.Wisp))
                {
                    var ModeLang = Main.IsWisping ? "Yes" : "No";
                    var ReadyLang = Main.CanWisp ? "Yes" : "No";
                    LowerInfoText.text = "Is Wisping: " + ModeLang;
                    LowerInfoText.text += "\nCan Wisp: " + ReadyLang;
                }
                else if (player.Is(CustomRoles.Pickpocket))
                {
            ...
            if (GameStates.IsInGame)
                if (!player.GetCustomRole().IsVanilla())
                {
                    ...
                    else
                    {
                        ...
                        else if (player.Is(CustomRoles.Predator))
                            TaskTextPrefix += Helpers.ColorString(player.GetRoleColor(), $"Kill {Options.PreysNeeded.GetInt()} players to win.");
                ...
                else
                {
                    switch (player.GetCustomRole())
                    {
                        ...
                        case CustomRoles.Engineer:
                            ...
                            {
                                TaskTextPrefix = Helpers.ColorString(player.GetRoleColor(), GetString("Role") + $": Hider\r\n");
                        ...
                        case CustomRoles.Impostor:
                            ...
                            {
                                TaskTextPrefix = Helpers.ColorString(player.GetRoleColor(), GetString("Role") + $": Seeker\r\n");
            ...
            if (cSubRoleFound)
            {
                ...
                else if (player.GetCustomSubRole() == CustomRoles.Chainlink)
                {
                    string name = "";
                    foreach (var chained in Main.ChainedPlayers)

                    {
                        if (chained.PlayerId == player.PlayerId) continue;
                        name = chained.GetRealName(true);
                    }
                    TaskTextPrefix += Helpers.ColorString(Utils.GetRoleColor(player.GetCustomSubRole()), $"You are linked with {name}.\nYou and {name} share a life.\r\n");
                }
            ...
            switch (player.GetCustomRole())
            {
                ...
                case CustomRoles.Investigator:
                    ...
                    goto DesyncImpostor;
                case CustomRoles.Consigliere:
                    if (!Consigliere.CanUseKillButton(player))
                    {
                        __instance.KillButton.SetDisabled();
                        __instance.KillButton.ToggleVisible(false);
                    }
    ...
    class ToggleHighlightPatch
    {
        public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] bool active, [HarmonyArgument(1)] RoleTeamTypes team)
        {
            ...
            if ((player.GetCustomRole() == CustomRoles.Sheriff ||
                ...
                player.GetCustomRole() == CustomRoles.Consigliere ||
                player.GetCustomRole() == CustomRoles.Cultist ||
                player.GetCustomRole() == CustomRoles.CorruptedSheriff ||
                player.GetCustomRole() == CustomRoles.Arsonist ||
                player.GetCustomRole() == CustomRoles.Traitor ||
                player.GetCustomRole() == CustomRoles.Jackal ||
                player.GetCustomRole() == CustomRoles.Mimicker ||
                player.GetCustomRole() == CustomRoles.Wraith ||
                player.GetCustomRole() == CustomRoles.Glitch ||
                ...
                player.GetCustomRole() == CustomRoles.NeutSerialKiller ||
                player.GetCustomRole() == CustomRoles.Vindicator ||
                player.GetCustomRole() == CustomRoles.NeutWitch ||
                player.GetCustomRole() == CustomRoles.Hitman ||
                player.GetCustomRole() == CustomRoles.Parasite ||
                player.GetCustomRole() == CustomRoles.PoisonMaster ||
                ...
                player.GetCustomRole() == CustomRoles.Predator ||
        ...
        public static void Postfix(HudManager __instance, [HarmonyArgument(2)] bool isActive)
        {
            ...
            switch (player.GetCustomRole())
            {
                ...
                case CustomRoles.Deputy:
                    ...
                    {
                        if (player.Data.Role.Role != RoleTypes.GuardianAngel)
                            __instance.KillButton.ToggleVisible(isActive && !player.Data.IsDead && Main.SheriffDied);
                        __instance.SabotageButton.ToggleVisible(false);
                        __instance.ImpostorVentButton.ToggleVisible(false);
                    }
                    else
                    {
                        if (player.Data.Role.Role != RoleTypes.GuardianAngel)
                            __instance.KillButton.ToggleVisible(isActive && !player.Data.IsDead && Sheriff.DeputyCanKill.GetBool());
                        __instance.SabotageButton.ToggleVisible(false);
                        __instance.ImpostorVentButton.ToggleVisible(false);
                    }
                    break;
                ...
                case CustomRoles.Mimicker:
                    if (player.Data.Role.Role != RoleTypes.GuardianAngel)
                        __instance.KillButton.ToggleVisible(isActive && !player.Data.IsDead);
                    __instance.SabotageButton.ToggleVisible(true);
                    __instance.ImpostorVentButton.ToggleVisible(isActive && Options.MimickerCanVent.GetBool());
                    //       __instance.AbilityButton.ToggleVisible(false);
                    break;
                case CustomRoles.Eradicator:
                    if (player.Data.Role.Role != RoleTypes.GuardianAngel)
                        //    __instance.KillButton.ToggleVisible(isActive && !player.Data.IsDead);
                        //    __instance.SabotageButton.ToggleVisible(true);
                        __instance.ImpostorVentButton.ToggleVisible(isActive && Options.EradicatorCanVent.GetBool());
                    //       __instance.AbilityButton.ToggleVisible(false);
                    break;
                ...
                case CustomRoles.NeutSerialKiller:
                    if (player.Data.Role.Role != RoleTypes.GuardianAngel)
                        __instance.KillButton.ToggleVisible(isActive && !player.Data.IsDead);
                    __instance.SabotageButton.ToggleVisible(true);
                    __instance.ImpostorVentButton.ToggleVisible(isActive && Options.SerialKillerCanVent.GetBool());
                    //     __instance.AbilityButton.ToggleVisible(false);
                    break;
                case CustomRoles.Traitor:
                    if (player.Data.Role.Role != RoleTypes.GuardianAngel)
                ...
                case CustomRoles.Vindicator:
                    if (player.Data.Role.Role != RoleTypes.GuardianAngel)
                        __instance.KillButton.ToggleVisible(isActive && !player.Data.IsDead);
                    __instance.SabotageButton.ToggleVisible(false);
                    __instance.ImpostorVentButton.ToggleVisible(isActive && Options.VindicatorCanVent.GetBool());
                    //     __instance.AbilityButton.ToggleVisible(false);
                    break;
                case CustomRoles.Parasite:
                    if (player.Data.Role.Role != RoleTypes.GuardianAngel)
                        __instance.KillButton.ToggleVisible(isActive && !player.Data.IsDead);
                    __instance.SabotageButton.ToggleVisible(false);
                    __instance.ImpostorVentButton.ToggleVisible(isActive && Options.ParasiteCanVent.GetBool());
                    //     __instance.AbilityButton.ToggleVisible(false);
                    break;
                case CustomRoles.PoisonMaster:
                    if (player.Data.Role.Role != RoleTypes.GuardianAngel)
    ...
    class ShowNormalMapPatch
    {
        ...
        {
            var player = PlayerControl.LocalPlayer;
            if (player.Is(CustomRoles.Sheriff) || player.Is(CustomRoles.Deputy) || player.Is(CustomRoles.BloodKnight) || player.Is(CustomRoles.NeutWitch) || player.Is(CustomRoles.Consigliere) || player.Is(CustomRoles.Cultist) || player.Is(CustomRoles.Escort) || player.Is(CustomRoles.Crusader) || player.Is(CustomRoles.Investigator) || player.Is(CustomRoles.Parasite) || player.Is(CustomRoles.Arsonist) || player.Is(CustomRoles.PlagueBearer) || player.Is(CustomRoles.TheGlitch) || player.Is(CustomRoles.Werewolf) || player.Is(CustomRoles.Opportunist) || player.Is(CustomRoles.Executioner) || player.Is(CustomRoles.Swapper) || player.Is(CustomRoles.Jester) || player.Is(CustomRoles.Pestilence))
            {
                __state = player.Data.Role.TeamType;
                player.Data.Role.TeamType = RoleTeamTypes.Crewmate;
            }
            if (player.Is(CustomRoles.CorruptedSheriff) || player.Is(CustomRoles.Wraith) || player.Is(CustomRoles.Glitch) || player.Is(CustomRoles.Traitor) || player.Is(CustomRoles.Mimicker) || player.Is(CustomRoles.NeutSerialKiller) || player.Is(CustomRoles.Saboteur))
            {
                __state = player.Data.Role.TeamType;
