    ...
    class ChatCommands
    {
            ...
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                switch (args[0])
                {
                    ...
                    case "/setmap":
                        ...
                        var numbereee = System.Convert.ToByte(subArgs);
                        GameOptionsManager.Instance.currentNormalGameOptions.MapId = numbereee;
                        break;
                    case "/m":
                    case "/myrole":
                        canceled = true;
                    ...
                    case "/roletype":
                        ...
                        {
                            case "crewmate":
                                Utils.SendMessage($"Host set their role type to {subArgs}");
                                //     PlayerControl.LocalPlayer.RpcSetCustomRole(CustomRoles.Crewmate);
                                PlayerControl.LocalPlayer.RpcSetRole(RoleTypes.Crewmate);
                                RoleManager.Instance.SetRole(PlayerControl.LocalPlayer, RoleTypes.Crewmate);
                                break;
                            ...
                            case "ga":
                                ...
                                break;
                            default:
                                Utils.SendMessage($"Host set their role type to Crewmate");
                                //     PlayerControl.LocalPlayer.RpcSetCustomRole(CustomRoles.Crewmate);
                                PlayerControl.LocalPlayer.RpcSetRole(RoleTypes.Crewmate);
                                break;
                        }
        ...
        public static void GetRolesInfo(string role)
        {
            var roleList = new Dictionary<CustomRoles, string>
            {
                ...
                { CustomRoles.Mercenary, "merc" },
                //{ CustomRoles.ShapeMaster, "sha" },
                { CustomRoles.TimeThief, "tt"},
                { CustomRoles.Trickster, "trick"},
                { CustomRoles.Pickpocket, "pi"},
          ...
          //      { CustomRoles.Wisp, "wis" },
                ...
                { CustomRoles.NeutSerialKiller, "sk" },
                { CustomRoles.Traitor, "trtr" },
                { CustomRoles.Vindicator, "vind" },
                { CustomRoles.PoisonMaster, "poison" },
                { CustomRoles.Survivor, "sur" },
                { CustomRoles.Wraith, "wra" },
                { CustomRoles.Glitch, "gli" },
                ...
                { CustomRoles.Bait, "ba" },
                { CustomRoles.Eternal, "eter" },
                { CustomRoles.Blind, "bli" },
                { CustomRoles.Tethered, "teth" },
                { CustomRoles.Oblivious, "obl" },
                { CustomRoles.DoubleShot, "ds" },
                { CustomRoles.Obvious, "obv" },
                 { CustomRoles.Sticky, "stba" },
                 { CustomRoles.Burst, "infe" },
                 { CustomRoles.Chainlink, "chli" },
                 { CustomRoles.QuickFix, "quick" },
                 { CustomRoles.Overclocked, "ovc" },
                 { CustomRoles.Amplify, "ampl" },
                 { CustomRoles.Insight, "insi" },
                 { CustomRoles.Outsight, "outsi" },
               { CustomRoles.Torch, "to" },
                { CustomRoles.Flash, "fl" },
                { CustomRoles.Bewilder, "be" },
                { CustomRoles.Forensic, "fore" },
                { CustomRoles.Necroview, "necrov" },
        ...
        public static void OnReceiveChat(PlayerControl player, string text)
        {
            ...
            switch (args[0])
            {
                ...
                case "/color":
                    ...
                    break;
                case "/c":
                    //  if (player.FriendCode is "gnuedaphic#7196" or "setloser#1264" or "croaktense#0572" or "primether#5348" or)
                    var friendcodes = ReturnAllNewLinesInFile(Main.SPECIALFRIENDCODES_FILE_PATH, noErr: true);
                    var friendcodess = ReturnAllNewLinesInFile(Main.TRUSTEDFRIENDCODES_FILE_PATH, noErr: true);
                    if (friendcodes.Contains(player.FriendCode) && AmongUsClient.Instance.AmHost || friendcodess.Contains(player.FriendCode) && AmongUsClient.Instance.AmHost)
                    {
                        subArgs = args.Length < 2 ? "" : args[1];
                        Utils.SendMessage("Color ID set to " + subArgs, player.PlayerId);
                        var numbere = System.Convert.ToByte(subArgs);
                    ...
                    else { Utils.SendMessage("You do not have access to this command.\nAsk the host to add your friend code to access this command.", player.PlayerId); }
                    break;
                case "/roleinfo":
