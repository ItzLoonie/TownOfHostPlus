        ...
        public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)] ref EndGameResult endGameResult)
        {
            ...
            if (GameManager.Instance.DidHumansWin(endGameResult.GameOverReason) || endGameResult.GameOverReason.Equals(GameOverReason.HumansByTask) || endGameResult.GameOverReason.Equals(GameOverReason.HumansByVote))
            {
                ...
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    ...
                    bool canWin = p.Is(RoleType.Crewmate) || !p.Is(RoleType.Madmate) || !p.Is(RoleType.Impostor);
            ...
            if (GameManager.Instance.DidImpostorsWin(endGameResult.GameOverReason))
            {
                ...
                {
                    //if (p.GetCustomSubRole() == CustomRoles.LoversRecode) continue;
                    bool canWin = p.Is(RoleType.Impostor) || p.Is(RoleType.Madmate) || p.Is(CustomRoles.CrewPostor) || p.Is(CustomRoles.CPSchrodingerCat) || p.Is(CustomRoles.CorruptedSheriff) || p.Is(CustomRoles.Undercover);
                    if (canWin) winner.Add(p);
                    if (canWin) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.ImpostorGhost);
                    if (!canWin) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.CrewmateGhost);
            ...
            if (Main.currentWinner == CustomWinner.Crewmate)
            {
                ...
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(RoleType.Crewmate) | !p.Is(CustomRoles.Undercover)) winner.Add(p);
                    if (!p.Is(RoleType.Crewmate) | p.Is(CustomRoles.Undercover)) winner.Remove(p);
                    if (p.Is(RoleType.Crewmate) | !p.Is(CustomRoles.Undercover)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.ImpostorGhost);
                    if (!p.Is(RoleType.Crewmate) | p.Is(CustomRoles.Undercover)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.CrewmateGhost);
                }
            }
            ...
            if (Main.currentWinner == CustomWinner.SerialKiller)
            {
                winner.Clear();
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(CustomRoles.NeutSerialKiller) | p.Is(CustomRoles.SKSchrodingerCat)) winner.Add(p);
                    if (p.Is(CustomRoles.NeutSerialKiller) | p.Is(CustomRoles.SKSchrodingerCat)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.ImpostorGhost);
                    if (!p.Is(CustomRoles.NeutSerialKiller) | !p.Is(CustomRoles.SKSchrodingerCat)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.CrewmateGhost);
                }
            }
            if (Main.currentWinner == CustomWinner.Vindicator)
            {
                winner.Clear();
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(CustomRoles.Vindicator) | p.Is(CustomRoles.VSchrodingerCat)) winner.Add(p);
                    if (p.Is(CustomRoles.Vindicator) | p.Is(CustomRoles.VSchrodingerCat)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.ImpostorGhost);
                    if (!p.Is(CustomRoles.Vindicator) | !p.Is(CustomRoles.VSchrodingerCat)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.CrewmateGhost);
                }
            }
            ...
            if (Main.currentWinner == CustomWinner.Glitch)
            {
                winner.Clear();
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(CustomRoles.Glitch)/* | p.Is(CustomRoles.GlitchSchrodingerCat)*/) winner.Add(p);
                    if (p.Is(CustomRoles.Glitch)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.ImpostorGhost);
                    if (!p.Is(CustomRoles.Glitch)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.CrewmateGhost);
                }
            }
            if (Main.currentWinner == CustomWinner.Traitor)
            {
                winner.Clear();
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(CustomRoles.Traitor)/* | p.Is(CustomRoles.TrSchrodingerCat)*/) winner.Add(p);
                    if (p.Is(CustomRoles.Traitor)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.ImpostorGhost);
                    if (!p.Is(CustomRoles.Traitor)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.CrewmateGhost);
                }
            }
            if (Main.currentWinner == CustomWinner.Mimicker)
            {
                winner.Clear();
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(CustomRoles.Mimicker)/* | p.Is(CustomRoles.MimicSchrodingerCat)*/) winner.Add(p);
                }
            }
            if (Main.currentWinner == CustomWinner.Predator)
            {
                winner.Clear();
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(CustomRoles.Predator)) winner.Add(p);
                    if (p.Is(CustomRoles.Predator)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.ImpostorGhost);
                    if (!p.Is(CustomRoles.Predator)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.CrewmateGhost);
                }
            }
            if (Main.currentWinner == CustomWinner.Egoist)
            {
                ...
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(CustomRoles.Egoist) | p.Is(CustomRoles.EgoSchrodingerCat)) winner.Add(p);
                }
            }
            if (Main.currentWinner == CustomWinner.Poisoner)
            {
                winner.Clear();
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(CustomRoles.PoisonMaster)/* | p.Is(CustomRoles.SKSchrodingerCat)*/) winner.Add(p);
                    if (p.Is(CustomRoles.PoisonMaster)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.ImpostorGhost);
                    if (!p.Is(CustomRoles.PoisonMaster)) p.RpcSetRole(AmongUs.GameOptions.RoleTypes.CrewmateGhost);
                }
            }
            ...
            if (Main.currentWinner == CustomWinner.TooManyPlayersLeft)
            {
                winner = new List<PlayerControl>();
                foreach (var p in PlayerControl.AllPlayerControls)
                {
        ...
        public static void Postfix(EndGameManager __instance)
        {
            ...
            switch (Main.currentWinner)
            {
                ...
                case CustomWinner.Crewmate:
                    ...
                    textRenderer.color = Utils.GetRoleColor(CustomRoles.Engineer);
                    break;
                case CustomWinner.Child:
                    CustomWinnerColor = Utils.GetRoleColorCode(CustomRoles.Child);
                    textRenderer.text = "Child died.";
                    __instance.BackgroundBar.material.color = Utils.GetRoleColor(CustomRoles.Child);
                ...
                case CustomWinner.Impostor:
                    ...
                    textRenderer.color = Utils.GetRoleColor(CustomRoles.Impostor);
                    break;
                case CustomWinner.Jackal:
                    CustomWinnerColor = Utils.GetRoleColorCode(CustomRoles.Jackal);
                    textRenderer.text = GetString("TeamJackal");
                    textRenderer.color = Utils.GetRoleColor(CustomRoles.Jackal);
                    break;
                case CustomWinner.Mimicker:
                    CustomWinnerColor = Utils.GetRoleColorCode(CustomRoles.Mimicker);
                    textRenderer.text = GetString("TeamMimicker");
                    textRenderer.color = Utils.GetRoleColor(CustomRoles.Mimicker);
                    break;
                ...
                case CustomWinner.Draw:
                    ...
                    break;
                case CustomWinner.TooManyPlayersLeft:
                    __instance.WinText.text = $"Too Many Players\nDisconnected";
                    //  __instance.WinText.color = Color.white;
                    __instance.BackgroundBar.material.color = Utils.GetRoleColor(CustomRoles.Impostor);
                    textRenderer.text = "Too many players left the game so the game ended";
            ...
            if (Main.currentWinner != CustomWinner.Draw && Main.currentWinner != CustomWinner.Crewmate && Main.currentWinner != CustomWinner.CrewmateDisconnected && Main.currentWinner != CustomWinner.Child && Main.currentWinner != CustomWinner.Dead && Main.currentWinner != CustomWinner.Impostor && Main.currentWinner != CustomWinner.Jackal && Main.currentWinner != CustomWinner.DisconnectError && Main.currentWinner != CustomWinner.TooManyPlayersLeft)
            {
                if (Main.currentWinner == CustomWinner.Crewmate && Main.currentWinner == CustomWinner.Impostor && Main.currentWinner == CustomWinner.Child && Main.currentWinner == CustomWinner.Jackal)
                {
                    textRenderer.text = $"<color={CustomWinnerColor}>Team {CustomWinnerText}{GetString("Win")}</color>";
                }
                else
                    textRenderer.text = $"<color={CustomWinnerColor}>{CustomWinnerText}{GetString("Win")}</color>";
            }
            ...
            foreach (var key in Main.AllPlayerCustomRoles)
            {
                try
                {
                    ...
                    if (killCountFound && killAmt != 0 && key.Value != CustomRoles.Pickpocket)
