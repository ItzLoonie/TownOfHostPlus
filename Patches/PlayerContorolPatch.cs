...
namespace TownOfHost
{
        ...
        public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
        {
            ...
            if (killer.PlayerId != target.PlayerId)
            {
                switch (killer.GetCustomRole())
                {
                    ...
                    case CustomRoles.Deputy:
                        if (!Main.SheriffDied)
                        {
                            if (!Sheriff.DeputyCanKill.GetBool())
                                return false;
                        }
                        else if (!Sheriff.CanUseKillButton(killer))
                            return false;
                        break;
            ...
            switch (target.GetCustomRole())
            {
                        ...
                        else
                        {
                            ...
                            switch (killer.GetCustomRole())
                            {
                                case CustomRoles.Egoist:
                                    ...
                                    break;
                                case CustomRoles.NeutSerialKiller:
                                    target.RpcSetCustomRole(CustomRoles.SKSchrodingerCat);
                                    break;
                                case CustomRoles.Vindicator:
                                    target.RpcSetCustomRole(CustomRoles.VSchrodingerCat);
                                    break;
                                case CustomRoles.Sidekick:
                                case CustomRoles.Jackal:
                ...
                case CustomRoles.MadGuardian:
                    ...
                    if (killer.Is(CustomRoles.Consigliere)
                    ) break;
                    if (killer.Is(CustomRoles.Cultist)
                    ) break;
                    ...
                    if (taskState.IsTaskFinished)
                    {
                        ...
                        NameColorManager.Instance.RpcAdd(killer.PlayerId, target.PlayerId, "#ff1313");
                        if (Options.MadGuardianCanSeeWhoTriedToKill.GetBool())
                            NameColorManager.Instance.RpcAdd(target.PlayerId, killer.PlayerId, "#ff1313");
                ...
                case CustomRoles.Ironclad:
                    ...
                    //MadGuardianを切れるかの判定処理
                    var taskStatee = target.GetPlayerTaskState();
                    if (taskStatee.IsTaskFinished)
                    {
                        int dataCountBefore = NameColorManager.Instance.NameColors.Count;
                        NameColorManager.Instance.RpcAdd(killer.PlayerId, target.PlayerId, "#ffffff");
                        if (Options.IroncladCanSeeWhoTriedToKill.GetBool())
                            NameColorManager.Instance.RpcAdd(target.PlayerId, killer.PlayerId, "#ff1313");
            ...
            if (killer.PlayerId != target.PlayerId)
            {
                ...
                if (killer.Is(CustomRoles.Mimicker) && target.Is(CustomRoles.Mimicker))
                {
                ...
                if (target.Is(CustomRoles.Wildling))
                {
                    if (!killer.Is(CustomRoles.Arsonist) && !killer.Is(CustomRoles.PlagueBearer) && !killer.Is(CustomRoles.Investigator) && Main.WildlingProtected) return false;
                }
                    ...
                    if (!killer.Is(CustomRoles.Arsonist))
                    {
                        if (!killer.Is(CustomRoles.PlagueBearer))
                        {
                            if (!killer.Is(CustomRoles.Investigator))
                            {
                                if (!killer.Is(CustomRoles.Consigliere))
                                {
                                    if (!killer.Is(CustomRoles.Saboteur))
                                    {
                                        if (!killer.Is(CustomRoles.Cultist))
                                        {
                                            killer.RpcGuardAndKill(killer);
                ...
                switch (killer.GetCustomRole())
                {
                    ...
                    case CustomRoles.Swooper:
                        ...
                        if (Main.IsInvis)
                        {
                            ...
                            if (target.Is(CustomRoles.Sticky) && !killer.Is(CustomRoles.Sticky))
                                killer.StickyKilled(target);

                            if (target.Is(CustomRoles.Demolitionist) && !killer.Is(CustomRoles.Demolitionist))
                                killer.DemoKilled(target);
                            if (target.Is(CustomRoles.Burst) && !killer.Is(CustomRoles.Burst))
                                killer.BurstKilled(target);
                    ...
                    case CustomRoles.Predator:
                        ...
                        if (!target.Is(CustomRoles.Masochist))
                        {
                            ...
                            Main.PreysKilled++;
                            MessageWriter writera = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetPredatorAmount, Hazel.SendOption.Reliable, -1);
                    ...
                    case CustomRoles.Eradicator:
                        ...
                        { //キルキャンセル&自爆処理
                          ...
                          // {
                            Utils.CustomSyncAllSettings();
                            Main.AllPlayerKillCooldown[killer.PlayerId] = Options.DefaultKillCooldown + Options.AcidCreateDuration.GetFloat();
                            killer.CustomSyncSettings(); //負荷軽減のため、killerだけがCustomSyncSettingsを実行
                            killer.RpcGuardAndKill(target);
                            Main.AcidatedPlayers.Add(target.PlayerId, (killer.PlayerId, 0f));
                            return false;
                            //  }
                        }
                        else
                    ...
                    case CustomRoles.Cultist:
                        ...
                        //   Main.WitchList[target.PlayerId] = killer.PlayerId;
                        Main.AllPlayerKillCooldown[killer.PlayerId] = Options.ConvertCooldown.GetFloat() * 1f;
                        killer.CustomSyncSettings(); // sync settings so they see.
                        killer.RpcGuardAndKill(target);
                        //  Main.MadmateConversions++;
                        //     MessageWriter writereee = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetNumOfMadmateConversionsLeft, Hazel.SendOption.Reliable, -1);
                        //   writereee.Write(Main.MadmateConversions);
                    ...
                    case CustomRoles.Consigliere:
                        ...
                        // if (Investigator.hasSeered[target.PlayerId]) if (!target.Is(CustomRoles.CorruptedSheriff)) return false;
                        //Investigator.OnCheckMurder(killer, target, Process: "RemoveShotLimit");
                        Consigliere.hasSeered[target.PlayerId] = true;
                        if (target.Is(CustomRoles.CorruptedSheriff))
                            Consigliere.SeeredCSheriff = true;
                        Logger.Info($"{killer.GetNameWithRole()} : Investigated Player: {target.GetNameWithRole()}", "Investigated");
                        killer.RpcGuardAndKill(target);
                        Utils.CustomSyncAllSettings();
                        Utils.NotifyRoles();
                        MessageWriter writereeee = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SeeredPlayer, Hazel.SendOption.Reliable, -1);
                        writereeee.Write(target.PlayerId);
                        AmongUsClient.Instance.FinishRpcImmediately(writereeee);
                        return false;
                    case CustomRoles.Wildling:
                        if (!Main.WildlingProtected)
                        {
                            Main.WildlingProtected = true;
                            new LateTask(() =>
                            {
                                Main.WildlingProtected = false;
                            }, Options.WildlingProtectDuration.GetFloat(), "Wildling Duration");
                        }
                        break;
                    case CustomRoles.BloodKnight:
    ...
    class MurderPlayerPatch
    {
        ...
        public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
        {
            ...
            else if (target.Is(CustomRoles.Consigliere) && killer.PlayerId != target.PlayerId)
            {
                Consigliere.hasSeered = new();
            }
            // Last Impostor
            ...
            if (Main.LawyerTarget.ContainsValue(target.PlayerId))
            {
                List<byte> RemoveGAKey = new();
                foreach (var gaTarget in Main.LawyerTarget)
                {
                    var ga = Utils.GetPlayerById(gaTarget.Key);
                    if (ga == null) continue;
                    if (target.PlayerId == gaTarget.Value && !ga.Data.IsDead)
                    {
                        // CRoleGuardianAngelChangeRoles
                        if (ga.IsModClient())
                            ga.RpcSetCustomRole(Options.CRoleLawyerChangeRoles[Options.WhenClientDies.GetSelection()]); //対象がキルされたらオプションで設定した役職にする
                        else
                        {
                            if (Options.CRoleLawyerChangeRoles[Options.WhenClientDies.GetSelection()] != CustomRoles.Amnesiac)
                                ga.RpcSetCustomRole(Options.CRoleLawyerChangeRoles[Options.WhenClientDies.GetSelection()]); //対象がキルされたらオプションで設定した役職にする
                            else
                                ga.RpcSetCustomRole(Options.CRoleLawyerChangeRoles[2]);
                        }
                        RemoveGAKey.Add(gaTarget.Key);
                    }
                }
                foreach (var RemoveKey in RemoveGAKey)
                {
                    Main.LawyerTarget.Remove(RemoveKey);
                    RPC.RemoveLawyerKey(RemoveKey);
                }
            }
            ...
            if (target.GetCustomSubRole() == CustomRoles.Chainlink)
            {
                PlayerControl chained = Main.ChainedPlayers.ToArray().Where(pc => pc.PlayerId == target.PlayerId).FirstOrDefault();
    ...
    class ShapeshiftPatch
    {
        public static void Prefix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
        {
            ...
            {//変身したとき一番近い人をマッドメイトにする処理
                ...
                if (mpdistance.Count() != 0)
                {
                    ...
                    if (Main.GuardianAngelTarget.ContainsKey(target.PlayerId))
                        Main.GuardianAngelTarget.Remove(target.PlayerId);
                    if (Main.LawyerTarget.ContainsKey(target.PlayerId))
                        Main.LawyerTarget.Remove(target.PlayerId);
            ...
            if (shapeshifter.Is(CustomRoles.Mercenary) && !shapeshifter.Data.IsDead && Main.MercCanSuicide && shapeshifting)
            {
    ...
    class ReportDeadBodyPatch
    {
        public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] GameData.PlayerInfo target)
        {
            ...
            if (__instance.Is(CustomRoles.Wraith))
            {
                return false;
            }
            ...
            if (target != null) //Sleuth Report for Non-Buttons
            {
                if (__instance.GetCustomSubRole() == CustomRoles.Sleuth)
                {
                    ...
                    //Main.MayorUsedButtonCount[__instance.PlayerId] += 1;
                    if (killer.GetCustomRole() == CustomRoles.Glitch)
                    {
                        Utils.SendMessage("The body you reported was corrupted.\nNo role could be identified.", __instance.PlayerId);
                    }
                    else
            ...
            if (target != null) //Detective Report for Non-Buttons
            {
                var killer = Utils.GetPlayerById(Main.whoKilledWho[target.PlayerId]);
                if (__instance.GetCustomRole() == CustomRoles.Detective)
                {
                    //Main.MayorUsedButtonCount[__instance.PlayerId] += 1;
                    if (killer.PlayerId == target.PlayerId || killer.GetCustomRole() == CustomRoles.Trickster || killer.GetCustomRole() == CustomRoles.Wraith)
                    {

                        Utils.SendMessage("Inspection Results:\n\nRole: " + Utils.GetRoleName(target.GetCustomRole()) + "\nKiller: Unknown\n\nAnalysis: The " + Utils.GetRoleName(target.GetCustomRole()) + " was killed by an unknown killer.", __instance.PlayerId);

                    }
                    else if (killer.GetCustomRole() == CustomRoles.Glitch)
                    {

                        Utils.SendMessage("Inspection Results:\n\nRole: Unknown\nKiller: Glitch\n\nAnalysis: The body was corrupted by the Glitch, leaving no info on its role behind.", __instance.PlayerId);

                    }

                    else
                        Utils.SendMessage("Inspection Results:\n\nRole: " + Utils.GetRoleName(target.GetCustomRole()) + "\nKiller: " + Utils.GetRoleName(killer.GetCustomRole()) + "\n\nAnalysis: The " + Utils.GetRoleName(target.GetCustomRole()) + " was killed by the " + Utils.GetRoleName(killer.GetCustomRole()) + ".", __instance.PlayerId);
                    foreach (var ar in Main.DetectiveReported)
                    {
                        if (ar.Key != __instance.PlayerId) break;
                        if (ar.Value.Item1 != target.PlayerId) break;
                        // now we set it to true
                        var stuff = Main.DetectiveReported[__instance.PlayerId];
                        stuff.Item2 = true;
                        Main.DetectiveReported[__instance.PlayerId] = stuff;
                    }
                }
            }
            if (target != null) //Trickster Report for Non-Buttons
            {
                var killer = Utils.GetPlayerById(Main.whoKilledWho[target.PlayerId]);
                if (__instance.GetCustomRole() == CustomRoles.Trickster)
                {
                    //Main.MayorUsedButtonCount[__instance.PlayerId] += 1;
                    if (killer.PlayerId == target.PlayerId || killer.GetCustomRole() == CustomRoles.Trickster || killer.GetCustomRole() == CustomRoles.Wraith)
                    {
                    ...
                    else if (killer.GetCustomRole() == CustomRoles.Glitch)
                    {
                        ...
                        Utils.SendMessage("Inspection Results:\n\nRole: " + Utils.GetRoleName(target.GetCustomRole()) + "\nKiller: " + Utils.GetRoleName(killer.GetCustomRole()) + "\n\nAnalysis: The " + Utils.GetRoleName(target.GetCustomRole()) + " was killed by the " + Utils.GetRoleName(killer.GetCustomRole()) + ".", __instance.PlayerId);
                    foreach (var ar in Main.TricksterReported)
                    {
                        if (ar.Key != __instance.PlayerId) break;
                        if (ar.Value.Item1 != target.PlayerId) break;
                        // now we set it to true
                        var stuff = Main.TricksterReported[__instance.PlayerId];
                        stuff.Item2 = true;
                        Main.TricksterReported[__instance.PlayerId] = stuff;
                    }
                }
            }
            ...
            foreach (var bp in Main.AcidatedPlayers)
            {
                var eradicatorID = bp.Value.Item1;
                var acidated = Utils.GetPlayerById(bp.Key);

                if (!acidated.Is(CustomRoles.Veteran))
                {
                    if (!acidated.Data.IsDead)
                    {
                        PlayerControl eradicator = Utils.GetPlayerById(eradicatorID);
                        if (acidated.Is(CustomRoles.Pestilence))
                            PlayerState.SetDeathReason(eradicator.PlayerId, PlayerState.DeathReason.Acid);
                        else
                            PlayerState.SetDeathReason(acidated.PlayerId, PlayerState.DeathReason.Acid);
                        //Protectは強制的にはがす
                        // PlayerControl vampire = Utils.GetPlayerById(vampireID);
                        if (acidated.protectedByGuardian)
                        {
                            if (acidated.GetCustomRole().IsImpostor())
                                acidated.RpcSetRole(RoleTypes.ImpostorGhost);
                            if (acidated.GetCustomRole().IsDesyncRole())
                                acidated.RpcSetRole(RoleTypes.ImpostorGhost);
                            else
                                acidated.RpcSetRole(RoleTypes.CrewmateGhost);

                        }
                        if (acidated.Is(CustomRoles.Pestilence))
                            eradicator.RpcMurderPlayer(eradicator);
                        else if (acidated.Is(CustomRoles.Survivor))
                        {
                            Utils.CheckSurvivorVest(acidated, eradicator, false);
                        }
                        else
                            acidated.RpcMurderPlayer(acidated);
                        RPC.PlaySoundRPC(eradicatorID, Sounds.KillSound);
                        if (acidated.Is(CustomRoles.Demolitionist))
                            Main.KilledDemo.Add(eradicatorID);
                        if (acidated.GetCustomSubRole() == CustomRoles.Bewilder)
                            Main.KilledBewilder.Add(eradicatorID);
                        if (acidated.GetCustomSubRole() == CustomRoles.Diseased)
                            Main.KilledDiseased.Add(eradicatorID);
                        Logger.Info("Vampireに噛まれている" + acidated?.Data?.PlayerName + "を自爆させました。", "ReportDeadBody");
                    }
                    else
                        Logger.Info("Vampireに噛まれている" + acidated?.Data?.PlayerName + "はすでに死んでいました。", "ReportDeadBody");
                }
                else
                {
                    if (Main.VetIsAlerted)
                    {
                        PlayerState.SetDeathReason(eradicatorID, PlayerState.DeathReason.Kill);
                        //Protectは強制的にはがす
                        PlayerControl eradicator = Utils.GetPlayerById(eradicatorID);
                        if (eradicator.protectedByGuardian)
                            eradicator.RpcMurderPlayer(eradicator);
                        eradicator.RpcMurderPlayer(eradicator);
                        RPC.PlaySoundRPC(acidated.PlayerId, Sounds.KillSound);
                    }
                    else
                    {
                        if (!acidated.Data.IsDead)
    ...
    class FixedUpdatePatch
    {
        public static void Postfix(PlayerControl __instance)
        {
            ...
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                if (GameStates.IsInTask && CustomRoles.Vampire.IsEnable() || GameStates.IsInTask && CustomRoles.PoisonMaster.IsEnable())
                {
                    //Vampireの処理
                        ...
                        if (killTimer >= Options.VampireKillDelay.GetFloat())
                        {
                            ...
                            if (!bitten.Data.IsDead)
                            {
                                ...
                                if (vampirePC.IsAlive())
                                {
                                    ...
                                    if (bitten.Is(CustomRoles.Sticky))
                                        vampirePC.StickyKilled(bitten);

                                    if (bitten.Is(CustomRoles.Demolitionist))
                                        Main.KilledDemo.Add(vampireID);
                                    if (bitten.Is(CustomRoles.Burst))
                                        Main.KilledBurst.Add(vampireID);
                                    if (bitten.GetCustomSubRole() == CustomRoles.Bewilder)
                                        Main.KilledBewilder.Add(vampireID);
                                    if (bitten.GetCustomSubRole() == CustomRoles.Diseased)
                                        Main.KilledDiseased.Add(vampireID);
                ...
                if (GameStates.IsInTask && CustomRoles.Eradicator.IsEnable())
                {
                    //Vampireの処理
                    if (Main.AcidatedPlayers.ContainsKey(player.PlayerId))
                    {
                        //__instance:キルされる予定のプレイヤー
                        //main.AcidatedPlayers[__instance.PlayerId].Item1:キルしたプレイヤーのID
                        //main.AcidatedPlayers[__instance.PlayerId].Item2:キルするまでの秒数
                        byte vampireID = Main.AcidatedPlayers[player.PlayerId].Item1;
                        float killTimer = Main.AcidatedPlayers[player.PlayerId].Item2;
                        if (killTimer >= Options.AcidKillDelay.GetFloat())
                        {
                            var bitten = player;
                            if (!bitten.Data.IsDead)
                            {
                                PlayerState.SetDeathReason(bitten.PlayerId, PlayerState.DeathReason.Acid);
                                var vampirePC = Utils.GetPlayerById(vampireID);
                                if (!bitten.Is(CustomRoles.Bait))
                                {
                                    if (vampirePC.IsAlive())
                                    {
                                        ...
                                        else if (bitten.Is(CustomRoles.Survivor))
                                        {
                                            foreach (var ar in Main.SurvivorStuff)
                                            {
                                                ...
                                                {
                                                    {
                                                        if (bitten.GetCustomRole().IsImpostor())
                                                            bitten.RpcSetRole(RoleTypes.ImpostorGhost);
                                                        if (bitten.GetCustomRole().IsDesyncRole())
                                                            bitten.RpcSetRole(RoleTypes.ImpostorGhost);
                                                        else
                                                            bitten.RpcSetRole(RoleTypes.CrewmateGhost);
                            ...
                            Main.AcidatedPlayers.Remove(bitten.PlayerId);
                        }
                        else
                        {
                            Main.AcidatedPlayers[player.PlayerId] =
                            (vampireID, killTimer + Time.fixedDeltaTime);
                        }
                    }
                }
            ...
            {
                if (GameStates.IsInTask && !GameStates.IsLobby && (__instance.Is(CustomRoles.Sheriff) || __instance.Is(CustomRoles.Deputy) || __instance.Is(CustomRoles.NeutSerialKiller) || __instance.Is(CustomRoles.Predator) || __instance.Is(CustomRoles.Glitch) || __instance.Is(CustomRoles.Traitor) || __instance.Is(CustomRoles.Vindicator) || __instance.Is(CustomRoles.Mimicker) || __instance.Is(CustomRoles.PoisonMaster) || __instance.Is(CustomRoles.AgiTater) || __instance.Is(CustomRoles.NeutWitch) || __instance.Is(CustomRoles.Consigliere) || __instance.Is(CustomRoles.Investigator) || __instance.Is(CustomRoles.Escort) || __instance.Is(CustomRoles.Parasite) || __instance.Is(CustomRoles.Crusader) || __instance.Is(CustomRoles.Hitman) || __instance.Is(CustomRoles.Janitor) || __instance.Is(CustomRoles.Painter) || __instance.Is(CustomRoles.Marksman) || __instance.Is(CustomRoles.BloodKnight) || __instance.Is(CustomRoles.Sidekick) || __instance.Is(CustomRoles.CorruptedSheriff) || __instance.GetRoleType() == RoleType.Coven || __instance.Is(CustomRoles.Arsonist) || __instance.Is(CustomRoles.Werewolf) || __instance.Is(CustomRoles.Wraith) || __instance.Is(CustomRoles.TheGlitch) || __instance.Is(CustomRoles.Juggernaut) || __instance.Is(CustomRoles.PlagueBearer) || __instance.Is(CustomRoles.Pestilence) || __instance.Is(CustomRoles.Jackal)) && !__instance.Data.IsDead)
                {
            ...
            if (RoleText != null && __instance != null)
            {
                ...
                if (GameStates.IsInGame)
                {
                    ...
                    {
                        var targetCheck = target.GetCustomRole().CanBeFoundBySnitch();
                    ...
                    if (seer.GetCustomRole().IsSheriffTeam())
                    {
                        if (target.GetCustomRole().IsSheriffTeam())
                        {
                            if (!seer.Data.IsDead)
                            {
                                // TeamText += "\r\n";
                    ...
                    {
                        if (target.GetCustomRole().IsJackalTeam())
                        {
                            if (!seer.Data.IsDead)
                            {
                                // TeamText += "\r\n";
                    ...
                    if (seer.GetCustomSubRole().IsLover())
                    {
                        if (Options.LoversKnowRoleOfOtherLover.GetBool())
                        {
                            //so we gotta make it so they can see the role of their lover
                            if (target.GetCustomSubRole().IsLover())
                            {
                                if (!seer.Data.IsDead)
                                {
                                    // TeamText += "\r\n";
                                    if (target != seer)
                                    {
                                        //        if (!seer.GetCustomRole().IsImpostor() && !target.GetCustomRole().IsImpostor())
                                        {
                                            if (!Options.RolesLikeToU.GetBool())
                                                RealName = $"<size={fontSize}>{Helpers.ColorString(target.GetRoleColor(), target.GetRoleName())}</size>\n{RealName}";
                                            else
                    ...
                    var canFindSnitchRole = seer.GetCustomRole().IsImpostor() || //LocalPlayerがインポスター
                        (Options.SnitchCanFindNeutralKiller.GetBool() && seer.IsNeutralKiller()) || seer.Is(CustomRoles.Trickster);//or キル可能な第三陣営
                    ...
                    switch (seer.GetRoleType())
                    {
                        ...
                        case RoleType.Impostor:
                            ...
                            if (target.GetCustomSubRole() == CustomRoles.Undercover && Options.ImpostorsKnowUndercover.GetBool())
                                RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Impostor), RealName);
                    ...
                    switch (seer.GetCustomSubRole())
                    {
                        case CustomRoles.Undercover:
                            if (target.GetRoleType() == RoleType.Impostor && Options.UndercoverKnowsImpostors.GetBool())
                                RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Impostor), RealName);
                            break;
                    }

                    if (seer.Is(CustomRoles.Trickster))
                    { //targetがインポスターかつ自分自身
                        var found = false;
                        var update = false;
                        var arrows = "";
                        ...
                        if (found && target.AmOwner) Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Snitch)}>★</color>"; //Snitch警告を表示
                        ...
                        }
                        {
                            ...
                            target.GetPlayerTaskState().DoExpose) //変更対象のタスクが終わりそう)
                                RealName += Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Snitch), "★"); //変更対象にSnitchマークをつける
                        }
                    }
                    ...
                    if (seer.Is(RoleType.Madmate) && Options.MadmatesSeeMarshall.GetBool())
                    {
                        if (target.Is(CustomRoles.Marshall) &&
                                            target.GetPlayerTaskState().IsTaskFinished)
                            RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Marshall), RealName);
                    ...
                    if (seer.Is(CustomRoles.Necroview) && target.Data.IsDead)
                    {
                        if (target.Is(RoleType.Crewmate) && target.Data.IsDead)
                            RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.SpeedBooster), RealName);
                        if (target.Is(RoleType.Impostor) && target.Data.IsDead)
                            RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Impostor), RealName);
                        if (target.Is(RoleType.Madmate) && target.Data.IsDead)
                            RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Madmate), RealName);
                        if (target.Is(RoleType.Neutral) && target.Data.IsDead)
                            RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.SchrodingerCat), RealName);
                    ...
                    foreach (var TargetGA in Main.LawyerTarget)
                    {
                        //if (Options.)
                        if ((seer.PlayerId == TargetGA.Key || seer.Data.IsDead) && //seerがKey or Dead
                        target.PlayerId == TargetGA.Value) //targetがValue
                    ...
                    if (seer.Is(CustomRoles.Consigliere))
                    {
                        if (Consigliere.hasSeered[target.PlayerId] == true)
                        {
                            // Consigliere has Seered Player.

                            {
                                if (Consigliere.IsImp(target))
                                {
                                    if (target.GetCustomRole().IsImpostor())
                                    {
                                        RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Consigliere), RealName); //targetの名前をエゴイスト色で表示
                                    }
                                ...
                                if (Consigliere.IsCrew(target))
                                {
                                    if (target.GetCustomRole().IsCrewmate())
                                    {
                                        RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.SpeedBooster), RealName); //targetの名前をエゴイスト色で表示
                                    }
                                }
                                if (Consigliere.IsNeutral(target))
                                {
                                    if (target.GetCustomRole().IsNeutral())
                                    {
                                        RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.SchrodingerCat), RealName); //targetの名前をエゴイスト色で表示
                                    }
                                }
                                if (Consigliere.IsCoven(target))
                                {
                                    if (target.GetCustomRole().IsCoven())
                                    {
                                        RealName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.NeutWitch), RealName); //targetの名前をエゴイスト色で表示
                                    }
                                }
                            }
                        }
                    ...
                    if (seer.Is(CustomRoles.Forensic) && target.Data.IsDead && !seer.Data.IsDead)
                    {
                        if (!target.Data.Disconnected)
                            Mark += $"({Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Forensic), Utils.GetVitalText(target.PlayerId))})";
                    }
                    if (seer.Is(CustomRoles.Trickster) && target.Data.IsDead && !seer.Data.IsDead)
                    {
                        if (!target.Data.Disconnected)
                            Mark += $"({Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Trickster), Utils.GetVitalText(target.PlayerId))})";
                    }
                    ...
                    if (__instance.GetCustomSubRole() == CustomRoles.Chainlink && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Chainlink)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Chainlink)}>●</color>";
                    }
                    else if (__instance.GetCustomSubRole() == CustomRoles.Chainlink && PlayerControl.LocalPlayer.Data.IsDead)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Chainlink)}>●</color>";
                    }
                    ...
                    if (GameStates.IsInTask && Options.SnitchEnableTargetArrow.GetBool() && target.Is(CustomRoles.Snitch))
                    {
                        ...
                        if (TaskState.IsTaskFinished)
                        {
                            ...
                            {
                                var foundCheck =
                                    pc.GetCustomRole().CanBeFoundBySnitch() ||
                    ...
                    if (GameStates.IsInTask && Options.DetectiveArrow.GetBool() && target.Is(CustomRoles.Detective))
                    {
        ...
        public static void Choked(byte deathId = 0x7f, bool isExiled = false)
        {
            if (CustomRoles.Chainlink.IsEnable() && Main.isChainedDead == false)
            {
                foreach (var chainedPlayer in Main.ChainedPlayers)
                {
                    //生きていて死ぬ予定でなければスキップ
                    // .GetCustomSubRole() == CustomRoles.LoversRecode
                    if (!chainedPlayer.Data.IsDead && chainedPlayer.PlayerId != deathId) continue;

                    Main.isChainedDead = true;
                    //   if (!Options.LoversDieTogether.GetBool()) continue;
                    foreach (var partnerPlayer in Main.ChainedPlayers)
                    {
                        //本人ならスキップ
                        if (chainedPlayer.PlayerId == partnerPlayer.PlayerId) continue;

                        //残った恋人を全て殺す(2人以上可)
                        //生きていて死ぬ予定もない場合は心中
                        if (partnerPlayer.PlayerId != deathId && !partnerPlayer.Data.IsDead && !partnerPlayer.Is(CustomRoles.Pestilence))
                        {
                            PlayerState.SetDeathReason(partnerPlayer.PlayerId, PlayerState.DeathReason.Choked);
                            if (isExiled)
                                Main.AfterMeetingDeathPlayers.TryAdd(partnerPlayer.PlayerId, PlayerState.DeathReason.Choked);
                            else
                                partnerPlayer.RpcMurderPlayer(partnerPlayer);
                        }
                    }
        ...
        public static bool DetectiveArrowUpdate(PlayerControl seer, DeadBody target, bool updateFlag, bool coloredArrow)
        {
            var key = (seer.PlayerId, target.ParentId);
            if (Main.DeadPlayersThisRound.Contains(target.ParentId))
            {
    ...
    class EnterVentPatch
    {
        public static void Postfix(Vent __instance, [HarmonyArgument(0)] PlayerControl pc)
        {
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                if (pc.Is(CustomRoles.Eradicator))
                {
                    if (!Options.EradicatorCanVent.GetBool())
                    {
                        skipCheck = true;
                        pc.MyPhysics.RpcBootFromVent(__instance.Id);
                    }
                }
                if (pc.Is(CustomRoles.Wildling))
                {
                    if (!Options.WildlingCanVent.GetBool())
                    {
                        skipCheck = true;
                        pc.MyPhysics.RpcBootFromVent(__instance.Id);
                    }
                ...
                if (pc.Is(CustomRoles.Trickster))
                {
                    {
                        skipCheck = true;
                        pc.MyPhysics.RpcBootFromVent(__instance.Id);
                ...
                if (pc.Is(CustomRoles.Parasite) && !Options.ParasiteCanVent.GetBool())
                {
                    {
                        skipCheck = true;
                        pc.MyPhysics.RpcBootFromVent(__instance.Id);
                ...
                /*   if (pc.Is(CustomRoles.Wraith))
                   {
                       if (!Main.IsInvisible && Main.CanGoInvisible)
                           skipCheck = true;
                       if (Main.IsInvisible)
                       {
                           pc.MyPhysics.RpcBootFromVent(__instance.Id);
                           ...
                           Options.WraithCooldown.GetFloat(), "WraithCooldown", true);
                       }
                   } */
                if (pc.Is(CustomRoles.Wisp))
                {
                    if (!Main.IsWisping && Main.CanWisp)
                        skipCheck = true;
                    if (Main.IsWisping)
                    {
                        pc.MyPhysics.RpcBootFromVent(__instance.Id);
                        Main.IsWisping = false;
                        Utils.NotifyRoles(SpecifySeer: pc);
                        new LateTask(() =>
                        {
                            Main.CanWisp = true;
                        ...
                        Options.WispCooldown.GetFloat(), "WispCooldown", true);
                    }
                }
    ...
    class CoEnterVentPatch
    {
        public static bool Prefix(PlayerPhysics __instance, [HarmonyArgument(0)] int id)
        {
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                __instance.myPlayer.Is(CustomRoles.Consigliere) ||
                ...
                __instance.myPlayer.Is(CustomRoles.Saboteur) ||
                ...
                (__instance.myPlayer.Is(CustomRoles.NeutSerialKiller) && !Options.SerialKillerCanVent.GetBool()) ||
                (__instance.myPlayer.Is(CustomRoles.Vindicator) && !Options.VindicatorCanVent.GetBool()) ||
                ...
                /*     if (__instance.myPlayer.Is(CustomRoles.Wraith))
                     {
                         _ = new LateTask(() =>
                         {
                             if (!Main.IsInvisible && Main.CanGoInvisible)
                             {
                                 ...
                                 Options.WraithDuration.GetFloat(), "WraithDuration", true);
                             }
                             else
                             {
                                 __instance.myPlayer.MyPhysics.RpcBootFromVent(id);
                ...
                if (__instance.myPlayer.Is(CustomRoles.Wisp))
                {
                    _ = new LateTask(() =>
                    {
                        if (!Main.IsWisping && Main.CanWisp)
                        {
                            ...
                            Options.WispDuration.GetFloat(), "WispDuration", true);
                        }
                        else
                        {
                            __instance.myPlayer.MyPhysics.RpcBootFromVent(id);
            ...
            if (roleType is RoleTypes.CrewmateGhost or RoleTypes.ImpostorGhost)
            {
                foreach (var seer in PlayerControl.AllPlayerControls)
                {
                    ...
                    if ((self && targetIsKiller) || (!seerIsKiller && target.Is(RoleType.Impostor)))
                    {
                        ...
                        target.RpcSetDesyncRole(RoleTypes.ImpostorGhost, seer.GetClientId());
                    }
                    else
                    {
                        Logger.Info($"Desync {target.GetNameWithRole()} =>CrewmateGhost for{seer.GetNameWithRole()}", "PlayerControl.RpcSetRole");
                        target.RpcSetDesyncRole(RoleTypes.CrewmateGhost, seer.GetClientId());
                    }
                }
                return false;
            }
