...
namespace TownOfHost
{
    public static class SerialKiller
    {
        ...
        {
            Options.SetupRoleOptions(Id, CustomRoles.Mercenary, AmongUsExtensions.OptionType.Impostor);
            KillCooldown = CustomOption.Create(Id + 10, Color.white, "SerialKillerCooldown", AmongUsExtensions.OptionType.Impostor, 20f, 2.5f, 100f, 2.5f, Options.CustomRoleSpawnChances[CustomRoles.Mercenary]);
            TimeLimit = CustomOption.Create(Id + 11, Color.white, "SerialKillerLimit", AmongUsExtensions.OptionType.Impostor, 60f, 5f, 180f, 5f, Options.CustomRoleSpawnChances[CustomRoles.Mercenary]);
        }
        ...
        {
            if (killer.Is(CustomRoles.Mercenary))
            {
        ...
        {
            if (!player.Is(CustomRoles.Mercenary)) return; //以下、シリアルキラーのみ実行
