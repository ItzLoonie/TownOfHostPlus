...
namespace TownOfHost
{
    public static class Consigliere
    {
        ...
        public static CustomOption ConsigKillCooldown;
        ...
        {
            Options.SetupSingleRoleOptions(Id, CustomRoles.Consigliere, 1, AmongUsExtensions.OptionType.Impostor);
            ConsigKillCooldown = CustomOption.Create(Id + 10, Color.white, "ConsigCooldown", AmongUsExtensions.OptionType.Impostor, 45, 1, 990, 1, Options.CustomRoleSpawnChances[CustomRoles.Consigliere]);
            //    SpyShowsAsRed = CustomOption.Create(Id + 19, Color.white, "SpyShowsAsRed", AmongUsExtensions.OptionType.Crewmate, false, Options.CustomRoleSpawnChances[CustomRoles.Investigator]);
        ...
        public static void Add(byte playerId)
        {
            ...
            CurrentKillCooldown.Add(playerId, ConsigKillCooldown.GetFloat());

            if (!Main.ResetCamPlayerList.Contains(playerId))
                Main.ResetCamPlayerList.Add(playerId);

            Logger.Info($"{Utils.GetPlayerById(playerId)?.GetNameWithRole()} : Add Consigliere Role", "Consigliere");
        }
