...
namespace TownOfHost
{
    enum CustomRPC
    {
        ...
        SetNumOfMadmateConversionsLeft,
        RpcSetCleanerClean,
        RpcAddKill,
        ...
        public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
        {
            ...
            switch (rpcType)
            {
                ...
                case CustomRPC.SetLoversPlayers:
                    ...
                    break;
                case CustomRPC.SetChainedPlayers:
                    Main.ChainedPlayers.Clear();
                    int countt = reader.ReadInt32();
                    for (int i = 0; i < countt; i++)
                        Main.ChainedPlayers.Add(Utils.GetPlayerById(reader.ReadByte()));
                    break;
                ...
                case CustomRPC.SetLawyerTarget:
                    byte lawId = reader.ReadByte();
                    byte targetIdss = reader.ReadByte();
                    Main.GuardianAngelTarget[lawId] = targetIdss;
                    break;
                case CustomRPC.RemoveLawyerTarget:
                    byte Keyss = reader.ReadByte();
                    Main.GuardianAngelTarget.Remove(Keyss);
                    break;
                ...
                case CustomRPC.SetVultureAmount: // DONE
                    ...
                    break;
                case CustomRPC.SetPredatorAmount: // DONE
                    Main.PreysKilled = reader.ReadInt32();
                    break;
                ...
                case CustomRPC.NotifyDemoKill: // DONE
                    ...
                    break;
                case CustomRPC.NotifyBurstKill: // DONE
                    var removee = reader.ReadBoolean();
                    if (!removee)
                        Main.KilledBurst.Add(reader.ReadByte());
                    else
                        Main.KilledBurst.Remove(reader.ReadByte());
                    break;
    ...
    static class RPC
    {
            ...
            try
            {
                ...
                switch (Main.currentWinner)
                {
                    ...
                    case CustomWinner.Juggernaut:
                        JugWin();
                        break;
                    case CustomWinner.Predator:
                        PredatorWin();
                        break;
                    case CustomWinner.Coven:
                        CovenWin();
                        break;
                    case CustomWinner.SerialKiller:
                        SerialKillerWin();
                        break;
                    case CustomWinner.Poisoner:
                        PoisWin();
                        break;
                    case CustomWinner.Traitor:
                        TraitorWin();
                        break;
                    case CustomWinner.Vindicator:
                        VindicatorWin();
                        break;
        ...
        public static void MimickerWin()
        {
            Main.currentWinner = CustomWinner.Jackal;
            CustomWinTrigger(0);
        }
        ...
        public static void NewGlitchWin()
        {
            Main.currentWinner = CustomWinner.Glitch;
            CustomWinTrigger(0);
        }
        ...
        public static void SerialKillerWin()
        {
            Main.currentWinner = CustomWinner.SerialKiller;
            CustomWinTrigger(0);
        }
        public static void PoisWin()
        {
            Main.currentWinner = CustomWinner.Poisoner;
            CustomWinTrigger(0);
        }
        public static void TraitorWin()
        {
            Main.currentWinner = CustomWinner.Traitor;
            CustomWinTrigger(0);
        }
        public static void VindicatorWin()
        {
            Main.currentWinner = CustomWinner.Vindicator;
            CustomWinTrigger(0);
        }
        ...
        public static void PredatorWin()
        {
            Main.currentWinner = CustomWinner.Predator;
            CustomWinTrigger(0);
        }
        ...
        public static void TooManyPlayersLeft()
        {
            if (ShipStatus.Instance == null) return;
        ...
        public static void SyncChainedPlayers()
        {
            if (!AmongUsClient.Instance.AmHost) return;
            MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetChainedPlayers, Hazel.SendOption.Reliable, -1);
            writer.Write(Main.ChainedPlayers.Count);
        ...
        public static void SendLawyerTarget(byte gaId, byte targetId)
        {
            MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetLawyerTarget, Hazel.SendOption.Reliable, -1);
            writer.Write(gaId);
            writer.Write(targetId);
            AmongUsClient.Instance.FinishRpcImmediately(writer);
        }
        ...
        public static void RemoveLawyerKey(byte Key)
        {
            if (!AmongUsClient.Instance.AmHost) return;
            MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.RemoveLawyerTarget, Hazel.SendOption.Reliable, -1);
            writer.Write(Key);
            AmongUsClient.Instance.FinishRpcImmediately(writer);
        }
