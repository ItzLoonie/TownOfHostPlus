    ...
    public static class Utils
    {
        ...
        public static bool HasTasks(GameData.PlayerInfo p, bool ForRecompute = true)
        {
            ...
            else
            {
                ...
                if (cRoleFound)
                {
                    ...
                    if (cRole == CustomRoles.Predator) hasTasks = false;
                    ...
                    if (cRole == CustomRoles.Consigliere) hasTasks = false;
                    if (cRole == CustomRoles.Cultist) hasTasks = false;
                    ...
                    //   if (cRole == CustomRoles.Agent) hasTasks = false;
                    if (cRole == CustomRoles.Trickster) hasTasks = false;
                    ...
                    if (cRole == CustomRoles.NeutSerialKiller) hasTasks = false;
                    if (cRole == CustomRoles.Traitor) hasTasks = false;
                    if (cRole == CustomRoles.Vindicator) hasTasks = false;
                    ...
                    if (cRole == CustomRoles.Mimicker) hasTasks = false;
                    if (cRole == CustomRoles.Wraith) hasTasks = false;
                    if (cRole == CustomRoles.Glitch) hasTasks = false;
                    ...
                    if (cRole == CustomRoles.Saboteur) hasTasks = false;
        ...
        public static string GetProgressText(byte playerId, bool comms = false)
        {
            ...
            switch (realRole)
            {
                ...
                case CustomRoles.Predator:
                    ProgressText = Helpers.ColorString(GetRoleColor(CustomRoles.Predator), $"({Main.PreysKilled}/{Options.PreysNeeded.GetInt()})");
                    break;
            ...
            if (checkTasks)
            {
                ...
                if (taskState.hasTasks)
                {
                    if (Options.VisibleTaskCounter.GetBool())
                    {
                        string Completed = comms ? "?" : $"{taskState.CompletedTasksCount}";
                        ProgressText += Helpers.ColorString(GetRoleColor(CustomRoles.Crewmate), $"({Completed}/{taskState.AllTasksCount})");
                    ...
                    switch (role.GetRoleType())
                    {
                        ...
                        case RoleType.Neutral:
                            ...
                            if (Options.NEshowEvil.GetBool())
                                if (role is CustomRoles.Jester or CustomRoles.Terrorist or CustomRoles.Executioner or CustomRoles.Swapper or CustomRoles.Masochist or CustomRoles.Hacker or CustomRoles.Vulture) badPlayers.Add(pc);
                            break;
        ...
        public static void NotifyRoles(bool isMeeting = false, PlayerControl SpecifySeer = null, bool NoCache = false, bool ForceLoop = false, bool startOfMeeting = false)
        {
            ...
            foreach (var seer in seerList)
            {
                ...
                var canFindSnitchRole = seer.Data.IsDead || seer.GetCustomRole().IsImpostor() || //LocalPlayerがインポスター
                ...
                if (seer.Is(CustomRoles.Chainlink)) SelfMark += $"<color={GetRoleColorCode(CustomRoles.Chainlink)}>●</color>";
                ...
                if (seer.Is(CustomRoles.Spellcaster))
                {
                ...
                /*    if (seer.Is(CustomRoles.Wraith))
                    {
                        var ModeLang = Main.IsInvisible ? "Yes" : "No";
                        var ReadyLang = Main.CanGoInvisible ? "Yes" : "No";
                        SelfSuffix = "Invisible: " + ModeLang;
                        SelfSuffix += "\nInvisibility Ready: " + ReadyLang;
                    } */
                if (seer.Is(CustomRoles.Wisp))
                {
                    var ModeLang = Main.IsWisping ? "Yes" : "No";
                    var ReadyLang = Main.CanWisp ? "Yes" : "No";
                    SelfSuffix = "Wisping: " + ModeLang;
                    SelfSuffix += "\nWisp Ready: " + ReadyLang;
                }
                ...
                if (seer.Is(CustomRoles.Pickpocket))
                {
                ...
                bool SeerKnowsMadmates = false; //trueの時、インポスターの名前が赤色に見える
                ...
                if (seer.Is(CustomRoles.Detective))
                {
                ...
                if (seer.Data.IsDead //seerが死んでいる
                    ...
                    || seer.Is(CustomRoles.Necroview) //seerがドクター
                    || seer.Is(CustomRoles.Trickster) //seerがドクター
                    ...
                    || seer.Is(CustomRoles.Consigliere)
                    || seer.Is(CustomRoles.Cultist)
                ...
                )
                {
                    ...
                    foreach (var target in PlayerControl.AllPlayerControls)
                    {
                        ...
                        /*        if (seer.GetCustomRole().IsSheriffTeam() && !isMeeting)
                                {
                                ...
                                if (seer.GetCustomRole().IsJackalTeam() && !isMeeting)
                                {
                        ...
                        if (seer.GetCustomSubRole().IsLover() && !isMeeting)
                        {
                        ...
                        if (seer.Is(CustomRoles.Chainlink) && target.Is(CustomRoles.Chainlink))
                        {
                            TargetMark += $"<color={GetRoleColorCode(CustomRoles.Chainlink)}>●</color>";
                        }
                        //霊界からラバーズ視認
                        ...
                        else if (seer.Data.IsDead && !seer.Is(CustomRoles.Chainlink) && target.Is(CustomRoles.Chainlink))
                        {
                            TargetMark += $"<color={GetRoleColorCode(CustomRoles.Chainlink)}>●</color>";
                        }
                    ...
                    Main.WitchList.ContainsKey(target.PlayerId))
                        ...
                        if (seer.Is(RoleType.Impostor) && target.Is(CustomRoles.Undercover) && Options.ImpostorsKnowUndercover.GetBool())
                        {
                            TargetPlayerName = Helpers.ColorString(GetRoleColor(CustomRoles.Impostor), TargetPlayerName);
                        }
                        if (seer.Is(CustomRoles.Undercover) && target.Is(RoleType.Impostor) && Options.UndercoverKnowsImpostors.GetBool())
                        {
                            TargetPlayerName = Helpers.ColorString(GetRoleColor(CustomRoles.Impostor), TargetPlayerName);
                        }


                        /*    if (seer.Is(CustomRoles.Agent) && target.Is(CustomRoles.Spy))
                            {
                        ...
                        if (SeerKnowsImpostors) //Seerがインポスターが誰かわかる状態
                        {
                            ...
                            else
                            {
                                ...
                                var foundCheck = target.GetCustomRole().CanBeFoundBySnitch() || (snitchOption && target.IsNeutralKiller());
                                if (foundCheck)
                                    TargetPlayerName = Helpers.ColorString(target.GetRoleColor(), TargetPlayerName);
                            }
                        }
                        else if (SeerKnowsMadmates)
                        {
                            var foundCheck = target.GetCustomRole().IsMadmate();
                        ...
                        if (seer.Is(CustomRoles.Necroview) && target.Data.IsDead)
                        {
                            if (target.Is(RoleType.Crewmate) && target.Data.IsDead)
                                TargetPlayerName = Helpers.ColorString(GetRoleColor(CustomRoles.SpeedBooster), TargetPlayerName);
                            if (target.Is(RoleType.Impostor) && target.Data.IsDead)
                                TargetPlayerName = Helpers.ColorString(GetRoleColor(CustomRoles.Impostor), TargetPlayerName);
                            if (target.Is(RoleType.Madmate) && target.Data.IsDead)
                                TargetPlayerName = Helpers.ColorString(GetRoleColor(CustomRoles.Madmate), TargetPlayerName);
                            if (target.Is(RoleType.Neutral) && target.Data.IsDead)
                                TargetPlayerName = Helpers.ColorString(GetRoleColor(CustomRoles.SchrodingerCat), TargetPlayerName);
                        ...
                        if (seer.Is(CustomRoles.Snitch) && seer.GetPlayerTaskState().IsTaskFinished && target.GetCustomRole().CanBeFoundBySnitch())
                        {
                        ...
                        {
                            TargetPlayerName = Helpers.ColorString(GetRoleColor(CustomRoles.Child), TargetPlayerName);
                        }
                        ...
                        {
                            if ((seer.PlayerId == LawyerTarget.Key || seer.Data.IsDead) && //seerがKey or Dead
                            target.PlayerId == LawyerTarget.Value) //targetがValue
                                TargetPlayerName = Helpers.ColorString(GetRoleColor(target.GetCustomRole()), TargetPlayerName);
                        }
                        ...
                        if (seer.Is(CustomRoles.Consigliere))
                        {
                            if (Consigliere.hasSeered[target.PlayerId] == true)
                            {
                                if (Consigliere.IsImp(target))
                                {
                                    if (target.GetCustomRole().IsImpostor())
                                    {
                                        TargetPlayerName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Consigliere), TargetPlayerName); //targetの名前をエゴイスト色で表示
                                    }
                                ...
                                if (Consigliere.IsCoven(target))
                                {
                                    if (target.GetCustomRole().IsCoven())
                                    {
                                        TargetPlayerName = Helpers.ColorString(Utils.GetRoleColor(CustomRoles.NeutWitch), TargetPlayerName); //targetの名前をエゴイスト色で表示
                                    }
                        ...
                        if (seer.Is(CustomRoles.Forensic) && target.Data.IsDead && !seer.Data.IsDead)
                            TargetDeathReason = $"({Helpers.ColorString(GetRoleColor(CustomRoles.Forensic), GetVitalText(target.PlayerId))})";
                        if (seer.Is(CustomRoles.Trickster) && target.Data.IsDead && !seer.Data.IsDead)
                            TargetDeathReason = $"({Helpers.ColorString(GetRoleColor(CustomRoles.Trickster), GetVitalText(target.PlayerId))})";
        ...
        public static void AfterMeetingTasks()
        {
            ...
            foreach (var pc in PlayerControl.AllPlayerControls)
            {
                ...
                if (pc.Is(CustomRoles.Guesser) || pc.Is(CustomRoles.Assassin) || pc.Is(CustomRoles.Vigilante) || pc.Is(CustomRoles.Pirate) || pc.Is(CustomRoles.NeutSerialKiller) || pc.Is(CustomRoles.Parasite))
                {
